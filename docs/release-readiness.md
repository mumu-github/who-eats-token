...
| Privacy/security | `npm run test:protocol`, `npm run test:secret-scan`, `npm audit --audit-level=high --registry=https://registry.npmjs.org/`, docs and templates | Secret redaction checks in issue/PR templates and source secret scanning |
...
- `npm audit --audit-level=high --registry=https://registry.npmjs.org/` requires working network access to a registry with advisory endpoints; mirror registry failures are evidence gaps, not clean vulnerability results.
